...
const discount_savings = (sorted_items, items_and_prices, special_offers) => {
  ...
  for(const offer in special_offers){
    ...
    // const offer_item_count = sorted_items.lastIndexOf(offer_object['item']) - sorted_items.indexOf(offer_object['item'])
    const offer_item_count = count_items(offer_object['item'], sorted_items)
    console.log(sorted_items)
    console.log(`offer object = ${offer_object['item']}`)
    console.log(`offer_item_count = ${offer_item_count}`)
    console.log(`required for offer = ${offer_object['quantity']}`)
    const number_of_times_offer_applies = offer_item_count / offer_object['quantity']
    console.log(`number_of_times_offer_applies = ${number_of_times_offer_applies}`)
...
const count_items = (item, items) => {
  ...
  const index_difference = items.lastIndexOf(item) - items.indexOf(item)
  if (index_difference === 0) return 1
