...
const discount_savings = (sorted_items, items_and_prices, special_offers) => {
  console.log(`in discount_savings`)
  let savings = 0

  Object.keys(special_offers).forEach((offer) => {
    if(!sorted_items.includes(offer.item)) { return }
    const number_of_items = sorted_items.lastIndexOf(offer.item) - sorted_items.indexOf(offer.item)
    const number_of_times_offer_applies = number_of_items / offer.quantity
    if (number_of_times_offer_applies <= 0) { return }
    savings += calculateSavings(offer, number_of_times_offer_applies, items_and_prices)
  })
